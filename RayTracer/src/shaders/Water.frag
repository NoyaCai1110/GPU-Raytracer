...
uniform vec3 mousePos;//tsingtao
...
Sphere sphere_water1 = initSphere(vec3(-0.7, -0.6, -2.5), 0.4, vec3(0.7, 0.7, 0.7), vec3(0.2, 0.9, 0.8), vec3(1.0, 1.0, 1.0), 0.15, 0.6, 0.35, 8, 0.0, 0.0, 0.0);
...
Triangle Triangle_floor1 = initTriangle(vec3(-floor_width, floor_depth, -20.0), vec3(-floor_width, floor_depth, 0.0), vec3(floor_width, floor_depth, -20.0), vec3(1.0, 1.0, 1.0), vec3(1.0, 0.8, 0.0), vec3(1.0, 1.0, 1.0), 0.15, 0.6, 0.35, 8, 0.0, 0.0, 0.0, false);
Triangle Triangle_floor2 = initTriangle(vec3(floor_width, floor_depth, 0.0), vec3(-floor_width, floor_depth, 0.0), vec3(floor_width, floor_depth, -20.0), vec3(1.0, 1.0, 1.0), vec3(1.0, 0.8, 0.0), vec3(1.0, 1.0, 1.0), 0.15, 0.6, 0.35, 8, 0.0, 0.0, 0.0, false);
Triangle Triangle_water1 = initTriangle(vec3(-water_width, water_depth, -10.0), vec3(-water_width, water_depth, 0.0), vec3(water_width, water_depth, -10.0), vec3(1.0, 1.0, 1.0), vec3(0.2, 0.6, 0.4), vec3(1.0, 1.0, 1.0), 0.075, 0.05, 0.5, 40, 0.2, 0.7, 1.08, true);
Triangle Triangle_water2 = initTriangle(vec3(water_width, water_depth, 0.0), vec3(-water_width, water_depth, 0.0), vec3(water_width, water_depth, -10.0), vec3(1.0, 1.0, 1.0), vec3(0.2, 0.6, 0.4), vec3(1.0, 1.0, 1.0), 0.075, 0.05, 0.5, 40, 0.2, 0.7, 1.08, true);
...
vec3 Gerstner_Circle_Wave (vec4 wave, vec3 p, inout vec3 tangent, inout vec3 binormal) {
    ...
    float steepness = wave.z * easecurve(1.0, 0.0, (time_t - timeStamp)/6);
    if((time_t - timeStamp)/6 > 1){
        steepness = 0;
    }
    float wavelength = wave.w;
    float k = 2 * PAI / wavelength;
...
vec3 cal_TriangleNormal(Ray ray, Triangle triangle, float t)
{
    ...
    if(triangle.iswave){
        ...
        vec4 MyWave1 = vec4(1.0, -2.0, 0.2, 0.3);
        vec4 MyWave2 = vec4(0.0, 1.0, 0.2, 0.3);
        vec4 MyWave3 = vec4(-2.0, 1.0, 0.2, 0.3);
        vec4 NoiseWave = vec4(randomVec(gridPoint.xz), 0.2, 0.5);
        GerstnerWave(MyWave1, gridPoint, tangent, binormal);
        GerstnerWave(MyWave2, gridPoint, tangent, binormal);
        GerstnerWave(MyWave3, gridPoint, tangent, binormal);
        ...
        vec4 CircleWave1 = vec4(circle_dir1, 0.5, 0.3);
        Gerstner_Circle_Wave(CircleWave1, gridPoint-centerPoint1, tangent, binormal);
        t_normal = normalize(cross(binormal, tangent));
        ...
        case 2: //Triangle
            ...
            for(int j = 0; j < l_num; j ++){    //multiple light
                ...
                if(t_list[sid].kr > 0){
                    vec3 dir_rfl = cal_dir_rfl(ray.direction, normal);
                    Ray rfl = initRay(ExtendedRay(ray, t), dir_rfl);
                    Color += t_list[sid].kr * Skycolor / l_num;
